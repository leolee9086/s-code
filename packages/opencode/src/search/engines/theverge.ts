/**
 * The Verge 科技新闻搜索引擎适配器
 *
 * 通过 DDG site:theverge.com 搜索。
 * 参考 SearXNG: 站点限定搜索模式
 * 零风险：不直接请求目标站点
 */
...
export function makeTheVerge(config: EngineConfig): SearchEngine {
  return {
    name: config.name,
    config,
    search: (http, query, opts) =>
      searchTheVerge(http, query, opts.numResults || config.maxResults, config.timeout),
  }
}

function searchTheVerge(
...
function parseDdgResults(html: string, max: number): SearchResult[] {
  ...
  while ((match = resultRe.exec(html)) !== null && results.length < max) {
    ...
    results.push(makeSearchResult({
      title: match[2].trim(),
      url: match[1],
      snippet: match[3].trim() || "The Verge",
      engine: "theverge",
      position: pos,
      category: "news",
    }))
...
export * as TheVergeEngine from "./theverge"
