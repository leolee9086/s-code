...
// TARGET_FILE 环境变量指定要分析的目标文件
const TARGET = process.env.TARGET_FILE
if (!TARGET) { console.error("请设置 TARGET_FILE 环境变量指向要分析的文件"); process.exit(1) }
...
function resolveImport(fromFile, specifier) {
  if (specifier.startsWith("/") || specifier.startsWith("node:") || specifier.startsWith("bun:") || !specifier.startsWith(".")) return null
